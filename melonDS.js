...
// refer to Module (if they choose; they can also define Module)
  ...
  (function() {
   var loadPackage = function(metadata) {
      ...
      var PACKAGE_NAME = '../../../melonDS.data';
      var REMOTE_PACKAGE_BASE = 'melonDS.data';
          ...
          DataRequest.prototype.byteArray = byteArray;
            ...
            for (var i = 0; i < files.length; ++i) {
              DataRequest.prototype.requests[files[i].filename].onload();
                ...
                Module['removeRunDependency']('datafile_../../../melonDS.data');

      };
      Module['addRunDependency']('datafile_../../../melonDS.data');
   ...
   loadPackage({"files": [{"filename": "/melonDS.ini", "start": 0, "end": 1396, "audio": 0}], "remote_package_size": 1396, "package_uuid": "eb73f51f-43b9-4430-967a-5cdb388e0904"});
...
var wasmBinaryFile = 'melonDS.wasm';
...
var ASM_CONSTS = {
  395008: function() {FS.syncfs(function (err) { assert(!err); console.log("IDBFS synchronized."); });},  
 395089: function() {let fb = document.getElementById('bios7'); if(fb) fb.style.display = 'block';},  
 395167: function() {let fb = document.getElementById('bios7'); if(fb) fb.style.display = 'none';},  
 395244: function() {let fb = document.getElementById('bios9'); if(fb) fb.style.display = 'block';},  
 395322: function() {let fb = document.getElementById('bios9'); if(fb) fb.style.display = 'none';},  
 395399: function() {let fb = document.getElementById('firmware'); if(fb) fb.style.display = 'block';},  
 395480: function() {let fb = document.getElementById('firmware'); if(fb) fb.style.display = 'none';},  
 395560: function() {let fb = document.getElementById('needed'); if(fb) fb.style.display = 'none';},  
 395638: function() {function dragover_handler(event) { event.preventDefault(); event.dataTransfer.dropEffect = 'move'; } var romPath = ""; function dropHandler(event) { event.preventDefault(); let loadedFiles = 0; let maxFiles = event.dataTransfer.files.length; const file = event.dataTransfer.files[0]; let reader = new FileReader(); reader.readAsArrayBuffer(file); reader.onload = function() { let data = new Uint8Array(reader.result); console.log("filename " + file.name); let filename = file.name; romPath = "" + file.name; let stream = FS.open(filename, 'w+'); FS.write(stream, data, 0, data.length, 0); FS.close(stream); console.log("Load complete."); var ptr = allocate(intArrayFromString(file.name), 'i8', ALLOC_NORMAL); Module.ccall('file_drag', 'number', ['string'], [file.name]); _free(ptr); console.log("Loaded file " + loadedFiles.toString() + "/" + maxFiles.toString()); }; } let em_canvas = document.getElementById('canvas'); em_canvas.addEventListener('dragover', dragover_handler, false); em_canvas.addEventListener('drop', dropHandler, false); let fs_button = document.getElementById('fullscreen'); if(fs_button) { fs_button.onclick = function() { Module.ccall('triggerFullscreen'); } } let save_button = document.getElementById('save'); if(save_button) { save_button.onclick = function() { FS.syncfs(function (err) { assert(!err); console.log("IDBFS synchronized."); }); } } let download_button = document.getElementById('downloadsave'); if(download_button) { download_button.onclick = function() { if(romPath !== "") { var saveFile = romPath.substr(0, romPath.lastIndexOf(".")) + ".sav"; var savPath = "saved/" + saveFile; alert("download rom save " + savPath); let content = FS.readFile(savPath); var a = document.createElement('a'); a.download = saveFile; a.href = URL.createObjectURL(new Blob([content], {type: 'application/octet-stream'})); a.style.display = 'none'; document.body.appendChild(a); a.click(); setTimeout(() => { document.body.removeChild(a); URL.revokeObjectURL(a.href); }, 2000); } } } let open_button = document.getElementById('opensave'); let filebrowser = document.getElementById('inputsave'); if(filebrowser) { filebrowser.addEventListener("change", function () { const fileList = this.files; for (let i = 0, numFiles = fileList.length; i < numFiles; i++) { const file = fileList[i]; var savFile = "saved/" + file.name; var fr = new FileReader(); fr.onload = function () { var data = new Uint8Array(fr.result); FS.writeFile(savFile, data); console.log("saved file " + savFile); FS.syncfs(function (err) { assert(!err); }); }; fr.readAsArrayBuffer(file); } }, false); } if(open_button && filebrowser) { open_button.onclick = function() { filebrowser.click(); } } function IntCombo(element, func) { let combo = document.getElementById(element); if(combo) { combo.onchange = function() { Module.ccall(func, 'number', ['number'], [combo.selectedIndex]); } } } IntCombo('size', 'onChangeScreenSize'); IntCombo('rotation', 'onChangeScreenRotation'); IntCombo('gap', 'onChangeScreenGap'); IntCombo('layout', 'onChangeScreenLayout'); IntCombo('filtering', 'onChangeScreenFiltering'); let check_swap = document.getElementById('swap'); if(check_swap) { check_swap.onchange = function() { Module.ccall('onChangeScreenSwap', 'number', ['number'], [check_swap.checked]); } } let slider_audio = document.getElementById('volume'); if(slider_audio) { slider_audio.onchange = function() { Module.ccall('onChangeAudioVolume', 'number', ['number'], [slider_audio.value]); } }},  
 399119: function() {setInterval(function() { Module.ccall('sync_check'); }, 200);},  
 399181: function() {FS.mkdir('/saved'); FS.mount(IDBFS, {}, '/saved'); FS.syncfs(true, function (err) { assert(!err); }); window.addEventListener("beforeunload", function (event) { FS.syncfs(function (err) { assert(!err); }); });},  
 399391: function() {setTimeout(function(){ Module.ccall('check_required_files'); }, 500);},  
 399461: function($0, $1, $2) {var w = $0; var h = $1; var pixels = $2; if (!Module['SDL2']) Module['SDL2'] = {}; var SDL2 = Module['SDL2']; if (SDL2.ctxCanvas !== Module['canvas']) { SDL2.ctx = Module['createContext'](Module['canvas'], false, true); SDL2.ctxCanvas = Module['canvas']; } if (SDL2.w !== w || SDL2.h !== h || SDL2.imageCtx !== SDL2.ctx) { SDL2.image = SDL2.ctx.createImageData(w, h); SDL2.w = w; SDL2.h = h; SDL2.imageCtx = SDL2.ctx; } var data = SDL2.image.data; var src = pixels >> 2; var dst = 0; var num; if (typeof CanvasPixelArray !== 'undefined' && data instanceof CanvasPixelArray) { num = data.length; while (dst < num) { var val = HEAP32[src]; data[dst ] = val & 0xff; data[dst+1] = (val >> 8) & 0xff; data[dst+2] = (val >> 16) & 0xff; data[dst+3] = 0xff; src++; dst += 4; } } else { if (SDL2.data32Data !== data) { SDL2.data32 = new Int32Array(data.buffer); SDL2.data8 = new Uint8Array(data.buffer); } var data32 = SDL2.data32; num = data32.length; data32.set(HEAP32.subarray(src, src + num)); var data8 = SDL2.data8; var i = 3; var j = i + 4*num; if (num % 8 == 0) { while (i < j) { data8[i] = 0xff; i = i + 4 | 0; data8[i] = 0xff; i = i + 4 | 0; data8[i] = 0xff; i = i + 4 | 0; data8[i] = 0xff; i = i + 4 | 0; data8[i] = 0xff; i = i + 4 | 0; data8[i] = 0xff; i = i + 4 | 0; data8[i] = 0xff; i = i + 4 | 0; data8[i] = 0xff; i = i + 4 | 0; } } else { while (i < j) { data8[i] = 0xff; i = i + 4 | 0; } } } SDL2.ctx.putImageData(SDL2.image, 0, 0); return 0;},  
 400916: function($0, $1, $2, $3, $4) {var w = $0; var h = $1; var hot_x = $2; var hot_y = $3; var pixels = $4; var canvas = document.createElement("canvas"); canvas.width = w; canvas.height = h; var ctx = canvas.getContext("2d"); var image = ctx.createImageData(w, h); var data = image.data; var src = pixels >> 2; var dst = 0; var num; if (typeof CanvasPixelArray !== 'undefined' && data instanceof CanvasPixelArray) { num = data.length; while (dst < num) { var val = HEAP32[src]; data[dst ] = val & 0xff; data[dst+1] = (val >> 8) & 0xff; data[dst+2] = (val >> 16) & 0xff; data[dst+3] = (val >> 24) & 0xff; src++; dst += 4; } } else { var data32 = new Int32Array(data.buffer); num = data32.length; data32.set(HEAP32.subarray(src, src + num)); } ctx.putImageData(image, 0, 0); var url = hot_x === 0 && hot_y === 0 ? "url(" + canvas.toDataURL() + "), auto" : "url(" + canvas.toDataURL() + ") " + hot_x + " " + hot_y + ", auto"; var urlBuf = _malloc(url.length + 1); stringToUTF8(url, urlBuf, url.length + 1); return urlBuf;},  
 401905: function($0) {if (Module['canvas']) { Module['canvas'].style['cursor'] = UTF8ToString($0); } return 0;},  
 401998: function() {if (Module['canvas']) { Module['canvas'].style['cursor'] = 'none'; }},  
 402067: function() {return screen.width;},  
 402092: function() {return screen.height;},  
 402118: function() {return window.innerWidth;},  
 402148: function() {return window.innerHeight;},  
 402179: function($0) {if (typeof setWindowTitle !== 'undefined') { setWindowTitle(UTF8ToString($0)); } return 0;},  
 402274: function() {if (typeof(AudioContext) !== 'undefined') { return 1; } else if (typeof(webkitAudioContext) !== 'undefined') { return 1; } return 0;},  
 402411: function() {if ((typeof(navigator.mediaDevices) !== 'undefined') && (typeof(navigator.mediaDevices.getUserMedia) !== 'undefined')) { return 1; } else if (typeof(navigator.webkitGetUserMedia) !== 'undefined') { return 1; } return 0;},  
 402635: function($0) {if(typeof(Module['SDL2']) === 'undefined') { Module['SDL2'] = {}; } var SDL2 = Module['SDL2']; if (!$0) { SDL2.audio = {}; } else { SDL2.capture = {}; } if (!SDL2.audioContext) { if (typeof(AudioContext) !== 'undefined') { SDL2.audioContext = new AudioContext(); } else if (typeof(webkitAudioContext) !== 'undefined') { SDL2.audioContext = new webkitAudioContext(); } if (SDL2.audioContext) { autoResumeAudioContext(SDL2.audioContext); } } return SDL2.audioContext === undefined ? -1 : 0;},  
 403128: function() {var SDL2 = Module['SDL2']; return SDL2.audioContext.sampleRate;},  
 403196: function($0, $1, $2, $3) {var SDL2 = Module['SDL2']; var have_microphone = function(stream) { if (SDL2.capture.silenceTimer !== undefined) { clearTimeout(SDL2.capture.silenceTimer); SDL2.capture.silenceTimer = undefined; } SDL2.capture.mediaStreamNode = SDL2.audioContext.createMediaStreamSource(stream); SDL2.capture.scriptProcessorNode = SDL2.audioContext.createScriptProcessor($1, $0, 1); SDL2.capture.scriptProcessorNode.onaudioprocess = function(audioProcessingEvent) { if ((SDL2 === undefined) || (SDL2.capture === undefined)) { return; } audioProcessingEvent.outputBuffer.getChannelData(0).fill(0.0); SDL2.capture.currentCaptureBuffer = audioProcessingEvent.inputBuffer; dynCall('vi', $2, [$3]); }; SDL2.capture.mediaStreamNode.connect(SDL2.capture.scriptProcessorNode); SDL2.capture.scriptProcessorNode.connect(SDL2.audioContext.destination); SDL2.capture.stream = stream; }; var no_microphone = function(error) { }; SDL2.capture.silenceBuffer = SDL2.audioContext.createBuffer($0, $1, SDL2.audioContext.sampleRate); SDL2.capture.silenceBuffer.getChannelData(0).fill(0.0); var silence_callback = function() { SDL2.capture.currentCaptureBuffer = SDL2.capture.silenceBuffer; dynCall('vi', $2, [$3]); }; SDL2.capture.silenceTimer = setTimeout(silence_callback, ($1 / SDL2.audioContext.sampleRate) * 1000); if ((navigator.mediaDevices !== undefined) && (navigator.mediaDevices.getUserMedia !== undefined)) { navigator.mediaDevices.getUserMedia({ audio: true, video: false }).then(have_microphone).catch(no_microphone); } else if (navigator.webkitGetUserMedia !== undefined) { navigator.webkitGetUserMedia({ audio: true, video: false }, have_microphone, no_microphone); }},  
 404848: function($0, $1, $2, $3) {var SDL2 = Module['SDL2']; SDL2.audio.scriptProcessorNode = SDL2.audioContext['createScriptProcessor']($1, 0, $0); SDL2.audio.scriptProcessorNode['onaudioprocess'] = function (e) { if ((SDL2 === undefined) || (SDL2.audio === undefined)) { return; } SDL2.audio.currentOutputBuffer = e['outputBuffer']; dynCall('vi', $2, [$3]); }; SDL2.audio.scriptProcessorNode['connect'](SDL2.audioContext['destination']);},  
 405258: function($0, $1) {var SDL2 = Module['SDL2']; var numChannels = SDL2.capture.currentCaptureBuffer.numberOfChannels; for (var c = 0; c < numChannels; ++c) { var channelData = SDL2.capture.currentCaptureBuffer.getChannelData(c); if (channelData.length != $1) { throw 'Web Audio capture buffer length mismatch! Destination size: ' + channelData.length + ' samples vs expected ' + $1 + ' samples!'; } if (numChannels == 1) { for (var j = 0; j < $1; ++j) { setValue($0 + (j * 4), channelData[j], 'float'); } } else { for (var j = 0; j < $1; ++j) { setValue($0 + (((j * numChannels) + c) * 4), channelData[j], 'float'); } } }},  
 405863: function($0, $1) {var SDL2 = Module['SDL2']; var numChannels = SDL2.audio.currentOutputBuffer['numberOfChannels']; for (var c = 0; c < numChannels; ++c) { var channelData = SDL2.audio.currentOutputBuffer['getChannelData'](c); if (channelData.length != $1) { throw 'Web Audio output buffer length mismatch! Destination size: ' + channelData.length + ' samples vs expected ' + $1 + ' samples!'; } for (var j = 0; j < $1; ++j) { channelData[j] = HEAPF32[$0 + ((j*numChannels + c) << 2) >> 2]; } }},  
 406343: function($0) {var SDL2 = Module['SDL2']; if ($0) { if (SDL2.capture.silenceTimer !== undefined) { clearTimeout(SDL2.capture.silenceTimer); } if (SDL2.capture.stream !== undefined) { var tracks = SDL2.capture.stream.getAudioTracks(); for (var i = 0; i < tracks.length; i++) { SDL2.capture.stream.removeTrack(tracks[i]); } SDL2.capture.stream = undefined; } if (SDL2.capture.scriptProcessorNode !== undefined) { SDL2.capture.scriptProcessorNode.onaudioprocess = function(audioProcessingEvent) {}; SDL2.capture.scriptProcessorNode.disconnect(); SDL2.capture.scriptProcessorNode = undefined; } if (SDL2.capture.mediaStreamNode !== undefined) { SDL2.capture.mediaStreamNode.disconnect(); SDL2.capture.mediaStreamNode = undefined; } if (SDL2.capture.silenceBuffer !== undefined) { SDL2.capture.silenceBuffer = undefined } SDL2.capture = undefined; } else { if (SDL2.audio.scriptProcessorNode != undefined) { SDL2.audio.scriptProcessorNode.disconnect(); SDL2.audio.scriptProcessorNode = undefined; } SDL2.audio = undefined; } if ((SDL2.audioContext !== undefined) && (SDL2.audio === undefined) && (SDL2.capture === undefined)) { SDL2.audioContext.close(); SDL2.audioContext = undefined; }},  
 407515: function() {throw 'Canceled!'},  
 407533: function($0, $1) {setTimeout(function() { __emscripten_do_dispatch_to_thread($0, $1); }, 0);}
};
function initPthreadsJS(){ PThread.initRuntime(); }
  ...
  var PThread={unusedWorkers:[],runningWorkers:[],tlsInitFunctions:[],initMainThreadBlock:function() {
        ...
        var pthreadMainJs = locateFile('melonDS.worker.js');
  ...
  if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(7, 1, fd, buf, len, flags, addr, addrlen);
  try {
  ...
  if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(8, 1, fd, message, length, flags, addr, addr_len);
  try {
  ...
  if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(10, 1, domain, type, protocol);
  try {
  ...
  if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(21, 1, display, majorVersion, minorVersion);
  ...
  if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(22, 1, display, draw, read, context);
  ...
  if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(23, 1, display, name);
  ...
  if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(25, 1, display, interval);
  ...
  if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(26, 1, display);
  ...
  if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(28, 1, nativeEngineId);
  ...
  if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(35, 1, index, gamepadState);
  ...
  if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(37, 1, target, deferUntilInEventHandler);
  ...
  if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(64, 1, title);
  ...
  if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(65, 1, __environ, environ_buf);
  try {
  ...
  if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(66, 1, penviron_count, penviron_buf_size);
  try {
...
/** @type {function(...*):?} */
var _file_drag = Module["_file_drag"] = function() {
  return (_file_drag = Module["_file_drag"] = Module["asm"]["file_drag"]).apply(null, arguments);
};

/** @type {function(...*):?} */
var _triggerFullscreen = Module["_triggerFullscreen"] = function() {
  return (_triggerFullscreen = Module["_triggerFullscreen"] = Module["asm"]["triggerFullscreen"]).apply(null, arguments);
};

/** @type {function(...*):?} */
var _sync_check = Module["_sync_check"] = function() {
  return (_sync_check = Module["_sync_check"] = Module["asm"]["sync_check"]).apply(null, arguments);
};

/** @type {function(...*):?} */
var _onChangeAudioVolume = Module["_onChangeAudioVolume"] = function() {
  return (_onChangeAudioVolume = Module["_onChangeAudioVolume"] = Module["asm"]["onChangeAudioVolume"]).apply(null, arguments);
};

/** @type {function(...*):?} */
var _onChangeScreenSize = Module["_onChangeScreenSize"] = function() {
  return (_onChangeScreenSize = Module["_onChangeScreenSize"] = Module["asm"]["onChangeScreenSize"]).apply(null, arguments);
};

/** @type {function(...*):?} */
var _onChangeScreenRotation = Module["_onChangeScreenRotation"] = function() {
  return (_onChangeScreenRotation = Module["_onChangeScreenRotation"] = Module["asm"]["onChangeScreenRotation"]).apply(null, arguments);
};

/** @type {function(...*):?} */
var _onChangeScreenGap = Module["_onChangeScreenGap"] = function() {
  return (_onChangeScreenGap = Module["_onChangeScreenGap"] = Module["asm"]["onChangeScreenGap"]).apply(null, arguments);
};

/** @type {function(...*):?} */
var _onChangeScreenLayout = Module["_onChangeScreenLayout"] = function() {
  return (_onChangeScreenLayout = Module["_onChangeScreenLayout"] = Module["asm"]["onChangeScreenLayout"]).apply(null, arguments);
};

/** @type {function(...*):?} */
var _onChangeScreenSwap = Module["_onChangeScreenSwap"] = function() {
  return (_onChangeScreenSwap = Module["_onChangeScreenSwap"] = Module["asm"]["onChangeScreenSwap"]).apply(null, arguments);
};

/** @type {function(...*):?} */
var _onChangeScreenFiltering = Module["_onChangeScreenFiltering"] = function() {
  return (_onChangeScreenFiltering = Module["_onChangeScreenFiltering"] = Module["asm"]["onChangeScreenFiltering"]).apply(null, arguments);
};

/** @type {function(...*):?} */
var _onChangeScreenSizing = Module["_onChangeScreenSizing"] = function() {
  return (_onChangeScreenSizing = Module["_onChangeScreenSizing"] = Module["asm"]["onChangeScreenSizing"]).apply(null, arguments);
};

/** @type {function(...*):?} */
var _check_required_files = Module["_check_required_files"] = function() {
  return (_check_required_files = Module["_check_required_files"] = Module["asm"]["check_required_files"]).apply(null, arguments);
};
...
var __emscripten_allow_main_runtime_queued_calls = Module['__emscripten_allow_main_runtime_queued_calls'] = 394840;
var __emscripten_main_thread_futex = Module['__emscripten_main_thread_futex'] = 7541120;
